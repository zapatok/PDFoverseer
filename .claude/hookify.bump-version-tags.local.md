...
action: block
conditions:
  - field: file_path, operator: regex_match, pattern: (core[/\\](pipeline|ocr|inference|image)\.py|vlm[/\\](client|parser|preprocess|benchmark)\.py)
---
...
1. **Choose which tag to bump** in `core/utils.py`:
   ...
   - `VLM_ENGINE_VERSION` — VLM client, parser, preprocessing, benchmark (`vlm/*.py`)
   - More than one if the change spans multiple areas

2. **Name the new version** (e.g. `s2t8-description`, `v2-description`, etc.)

3. **Edit `core/utils.py` FIRST** with the new version string (editing `core/utils.py` itself does not trigger this rule)

4. **Confirm to the user**: "Bumped [TAG]: old → new (reason)"

Only THEN retry the edit to the vital file.

Vital files: `core/{pipeline,ocr,inference,image}.py`, `vlm/{client,parser,preprocess,benchmark}.py`
