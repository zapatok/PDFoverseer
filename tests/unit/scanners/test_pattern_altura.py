"""Smoke tests for the altura pagination pattern (Task 5.9).
...
Fixture: 4 HRB altura PDFs, each 1 page.  Each is a separate height-work
checklist (diverse templates across contractors).  The PaginationScanner
applies the A7 lock: every single-page PDF counts as 1 document without OCR.
Expected cover count = 4.

Note: real corpus altura PDFs use centered pagination stamps (not the
...
def test_altura_count_ocr_smoke():
    """PaginationScanner returns 4 docs for the 4-file altura fixture folder.
    ...
    if not _any_fixture_pdf_present():
        pytest.skip("Altura fixture PDFs not present (gitignored)")
    ...
    assert result.count == gt["covers_expected"], (
        f"Altura cover count mismatch: got {result.count}, expected {gt['covers_expected']}. "
        f"per_file={result.per_file!r}  flags={result.flags!r}  errors={result.errors!r}"
    )


def test_altura_count_ocr_a7_lock_fires():
