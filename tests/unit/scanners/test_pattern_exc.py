"""Smoke tests for the exc anchor pattern (Task 5.8).
...
_FIXTURE_DIR = Path(__file__).parent.parent.parent / "fixtures" / "scanners" / "exc"
_PDF = _FIXTURE_DIR / "exc_chequeos.pdf"
...
def test_exc_count_ocr_smoke():
    """AnchorsScanner returns the expected cover count for the exc fixture."""
    if not _PDF.exists():
        pytest.skip("exc fixture PDF not present (gitignored)")

    gt = _load_gt()
    scanner = AnchorsScanner(sigla="exc")
...
def test_exc_count_ocr_per_file_breakdown():
    ...
    assert result.per_file[_PDF.name] == 23
