...
def test_all_r1_single_page_is_settled(tmp_path, make_pdf):
...
def test_a_pending_multipage_is_not_settled(tmp_path, make_pdf):
    make_pdf(tmp_path / "a.pdf", 1)
    make_pdf(tmp_path / "big.pdf", 8)  # filename_glob multipage → Pendiente
    cell = {
...
def test_ocr_file_is_not_settled(tmp_path, make_pdf):
    make_pdf(tmp_path / "a.pdf", 5)
    cell = {
        "per_file": {"a.pdf": 2},
...
def test_ratio_n_is_settled(tmp_path, make_pdf):
    make_pdf(tmp_path / "big.pdf", 8)
    cell = {
        "per_file": {"big.pdf": 4},
...
def test_pending_overridden_per_file_is_settled(tmp_path, make_pdf):
    make_pdf(tmp_path / "big.pdf", 8)
    cell = {
        "per_file": {"big.pdf": 1},
