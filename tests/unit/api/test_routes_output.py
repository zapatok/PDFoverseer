...
# tree (checks/charla/chintegral/dif_pts cells). Guard those with
# @pytest.mark.corpus. Tests that redirect INFORME_MENSUAL_ROOT to tmp_path, or
# never touch the client at all (pure _build_* helpers on a synthetic state
# dict), are corpus-independent and are left unguarded — see QA-3 audit.
