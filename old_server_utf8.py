...
async def api_start(req: StartProcessRequest):
    ...
    if req.start_index == 0:
        ...
        # Si un archivo NO est├í en _reads, fall├│ o fue saltado, as├¡ que lo sobreescribiremos. 
        # Los que ya est├ín listos no se tocan.
        pass
...
async def api_stop():
    ...
    state.pause_event.set()
    _emit("log", {"msg": "­ƒøæ Proceso detenido (abortando).", "level": "error"})
    return {"success": True}
...
async def api_skip():
    ...
    state.cancel_event.set()
    state.pause_event.set()
    _emit("log", {"msg": "ÔÅ¡ Saltando archivo actual...", "level": "warn"})
    return {"success": True}
...
async def api_pause():
    ...
    _emit("log", {"msg": "ÔÅ© An├ílisis en pausa.", "level": "warn"})
...
async def api_resume():
    ...
    if state.pause_start_time > 0:
        ...
        state.pause_start_time = 0.0
    _emit("log", {"msg": "ÔûÂ Reanudando an├ílisis...", "level": "ok"})
...
def _process_pdfs(start_index: int = 0):
    _emit("log", {"msg": "Pre-calculando p├íginas del lote...", "level": "info"})
    try:
        total_pages = 0
        done_pages = 0
        ...
        _emit("log", {"msg": f"Total a procesar: {total_pages} p├íginas.", "level": "success"})
    except Exception as e:
        _emit("log", {"msg": f"Error calculando p├íginas: {e}", "level": "error"})
        return

    _emit("log", {"msg": "Iniciando an├ílisis profundo...", "level": "section"})
    ...
    for idx in range(start_index, len(state.pdf_list)):
        ...
        if str(pdf_path) in state.pdf_reads:
            _emit("log", {"msg": f"\nÔÅ¡ Omitiendo {pdf_path.name} (Ya procesado al 100%).", "level": "warn"})
        ...
        _emit("log", {"msg": f"\n­ƒôä [{idx+1}/{len(state.pdf_list)}] {pdf_path.name}", "level": "file_hdr"})
        ...
        try:
            ...
            if state.stop_requested:
                on_log(f"An├ílisis abortado a petici├│n del usuario. Extrayendo datos parciales...", "warn")
                break
            ...
            # [UI:] line ÔÇö actual UI counters after _recalculate_metrics rebuild
            try:
                ...
                on_log(f"[UI:ERR] {pdf_path.name} ÔÇö {_ui_err!r}", "ai")
    ...
    _emit("log", {"msg": f"\n­ƒÅü Proceso completado en {elapsed_total:.1f}s", "level": "ok"})
...
def api_exclude(req: ExcludeRequest):
    ...
    _emit("log", {"msg": f"Excluyendo p├ígina {req.page} y recalculando {Path(pdf_str).name}...", "level": "info"})
    ...
    _recalculate_metrics()
    _emit("log", {"msg": "P├ígina excluida, actualizando m├®tricas...", "level": "ok"})
    with state._lock:
        surviving = [i for i in state.issues if i["pdf_path"] == pdf_str]
    _emit("issues_refresh", {
        "pdf_path": pdf_str,
