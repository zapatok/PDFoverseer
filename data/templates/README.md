...
| Sigla rows (canonical 20) | 10-28 | One row per sigla — see mapping below. `chps` (row 31) is counted but NOT written to the Excel (Incr B). |
| (formerly orphan) | 22, 26 | **Mapped in Incr B (2026-06-23)** to `revdocmaq` (22) / `espacios` (26). |
| Workforce inputs | 29-30 | Values live in the HH columns (H/J/L/N), not the cantidad columns |

### Named range conventions

**Cantidad cells (76 ranges)** — `<HOSPITAL>_<SIGLA>_count`:

- `<HOSPITAL>` ∈ `HLL`, `HLU`, `HRB`, `HPV`
- `<SIGLA>` ∈ the 19 *written* siglas (the 20 in `core/domain.py:SIGLAS` minus `chps`, which is counted but not written to the Excel — Incr B)
...
| revdocmaq | 22 | "Revisión de Documentación de maquinaria" *(mapped in Incr B)* |
...
| espacios | 26 | "Inspección de Medidas de Seguridad para trabajos en espacios confinados" *(mapped in Incr B)* |
| herramientas_elec | 27 | "Inspección de Herramientas Eléctricas" |
| andamios | 28 | "Inspección de Andamios" |
| chps | **31** | "CPHS — Comité Paritario de Higiene y Seguridad" — **label only; NOT written to the Excel** (counted + in history; Incr B "solo cphs no va al excel") |
