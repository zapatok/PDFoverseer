...
**Architecture:** Single new sweep script (`sweep_rescue.py`) runs all three investigations with a shared cross-validation harness over 26 PDFs. Each rescue line produces per-PDF count-only metrics (error, abs_error) compared to V1-count baseline. A final report summarizes which lines (if any) beat V1 on cross-validation MAE.
...
**Success criteria:** A rescue line is viable if it achieves **lower MAE than V1-count (20.3) on the 21-PDF general corpus** AND **lower MAE than V1-count (0.2) on the 5-PDF ART family**, OR if it achieves **comparable MAE with demonstrably better page-level accuracy on ART_674** (F1 > 0.922).
...
**PDF corpus for cross-validation (26 total):**
- 21 general PDFs in `data/samples/` (ALUM_1..SAEZ_14, HLL_363, ART_674)
...
The sweep needs a reusable function that, given a scoring function, runs it over all 26 PDFs and returns per-PDF count metrics. This avoids duplicating the PDF iteration loop for each rescue line.
...
def format_comparison_table(
    ...
    # Rows
    v1_by_name = {r["name"]: r for r in v1_results}
    rescue_by_name = {
...
Expected: Completes in ~10-15 minutes (26 PDFs x 4 scorer variants + Rescue B sweep of 4 weights). Prints comparative tables and saves `data/pixel_density/sweep_rescue.json`.
