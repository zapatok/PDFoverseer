...
`core/cell_count.py::compute_cell_count` se reestructura para hornear el delta **después** de resolver la cascada base — así el delta es aditivo y respeta el `user_override` como base:

```python
def compute_cell_count(cell, count_type="documents", present_files=None):
    base = _base_count(cell, count_type, present_files)   # cascada actual (override/checks/per_file/fallback)
    return base + (cell.get("reorg_doc_delta") or 0)
```

El espejo JS `frontend/src/lib/cellCount.js::computeCellCount` aplica el mismo `+ (cell.reorg_doc_delta ?? 0)`. La paridad cross-language se valida con casos nuevos en `tests/fixtures/cell_count_cases.json` (+ `tests/test_cell_count_cross_language.py`).

El delta fluye así **automáticamente** a UI, Excel (`core/excel/writer.py`) e histórico — los tres ya leen `compute_cell_count` (consolidación 2026-06-06). No se toca ningún llamador.

### Trabajadores
`compute_worker_count` (el total de trabajadores; alimenta el visor y N15 vía Incr 3B) suma `+ (cell.get("reorg_worker_delta") or 0)` análogamente. El delta de trabajadores es un **número**, no marcas reubicadas: las marcas reales se quedan con el archivo físico hasta que el paso 1 lo mueva.
...
### `refresh_reorg_deltas` (espejo de `refresh_all_reliable`)
Función nueva en `api/routes/sessions.py`:
...
- al **terminar un escaneo** (`POST /scan`) → `check_applied=True` (es el único momento en que un archivo pudo haberse movido físicamente).
...
- **Múltiples `extract_pages` del mismo archivo:** todas apuntan a la numeración del **original**. El contrato instruye al paso 1 a aplicarlas en orden de página **descendente** (o contra una copia intacta) para no correr los índices (§10).
...
- **Fuente única:** `state["reorg_ops"]`; deltas son cachés recomputados (espejo de `all_reliable`).
