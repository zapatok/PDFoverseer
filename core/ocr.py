...
from core.image import _deskew, _render_clip, clean_for_ocr
...
def _upsample_4x(bgr: np.ndarray) -> np.ndarray:
    ...
    if _gpu_sr_device is not None:
        ...
        t = torch.from_numpy(bgr).permute(2, 0, 1).float().unsqueeze(0).to(_gpu_sr_device) / 255.0
        t_up = F.interpolate(t, scale_factor=4, mode="bicubic", align_corners=False)
        return t_up.squeeze(0).permute(1, 2, 0).clamp(0, 1).mul(255).byte().cpu().numpy()
